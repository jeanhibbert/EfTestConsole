namespace EfTestConsole.Benchmarks
{
	...
	public abstract class BencherBase<T> : IBencher
	{
		...
		protected BencherBase(Func<T, int> salesOrderIdRetriever, bool usesChangeTracking, bool usesCaching, bool supportsEagerLoading=false)
		{
			...
			this._salesOrderIdRetriever = salesOrderIdRetriever;
			this.UsesCaching = usesCaching;
			this.UsesChangeTracking = usesChangeTracking;
			this.SupportsEagerLoading = supportsEagerLoading;
			this._individualBenchmarkResults = new List<BenchResult>();
			this._setBenchmarkResults = new List<BenchResult>();
			this._eagerLoadBenchmakResults = new List<BenchResult>();
			this._frameworkName = string.Empty;
		}
		...
		{
			if(string.IsNullOrEmpty(this._frameworkName))
			{
				this._frameworkName = this.CreateFrameworkNameImpl();
			}
		...
		{
			this._individualBenchmarkResults.Clear();
			this._setBenchmarkResults.Clear();
			this._eagerLoadBenchmakResults.Clear();
		...
		{
			var individualFetchValues = this._individualBenchmarkResults.Select(r => r.FetchTimeInMilliseconds).ToList();
			this.IndividualFetchMean = individualFetchValues.Count <= 0 ? -1.0 : individualFetchValues.Average();
			this.IndividualFetchSD = this.CalculateSD(individualFetchValues, this.IndividualFetchMean);

			var setFetchValues = this._setBenchmarkResults.Select(r => r.FetchTimeInMilliseconds).ToList();
			this.SetFetchMean = setFetchValues.Count <= 0 ? -1.0 : setFetchValues.Average();
			this.SetFetchSD = this.CalculateSD(setFetchValues, this.SetFetchMean);

			var enumerationValues = this._setBenchmarkResults.Select(r => r.EnumerationTimeInMilliseconds).ToList();
			this.EnumerationMean = enumerationValues.Count <= 0 ? -1.0 : enumerationValues.Average();
			this.EnumerationSD = this.CalculateSD(enumerationValues, this.EnumerationMean);

			if(this.SupportsEagerLoading)
			{
				var eagerLoadFetchValues = this._eagerLoadBenchmakResults.Select(r=>r.FetchTimeInMilliseconds).ToList();
				this.EagerLoadFetchMean = eagerLoadFetchValues.Count <= 0 ? -1.0 : eagerLoadFetchValues.Average();
				this.EagerLoadFetchSD = this.CalculateSD(eagerLoadFetchValues, this.EagerLoadFetchMean);
			}
		}
		...
		public BenchResult PerformIndividualBenchMark(List<int> keys, bool discardResults)
		{
			...
			{
				var element = this.FetchIndividual(key);
				var verifyResult = this.VerifyElement(element);
				if(verifyResult > 0)
				{
			...
			if (!discardResults)
			{
				this._individualBenchmarkResults.Add(toReturn);
			}
			return toReturn;
		}
		...
		{
			return this.PerformIndividualBenchMark(keys, discardResults: false);
		}
		...
		{
			return this.PerformSetBenchmark(discardResults: false);
		}
		...
		public BenchResult PerformSetBenchmark(bool discardResults)
		{
			...
			var sw = new Stopwatch();
			sw.Start();
			var headers = this.FetchSet();
			sw.Stop();
			toReturn.FetchTimeInMilliseconds = sw.Elapsed.TotalMilliseconds;
			sw.Reset();
			sw.Start();
			toReturn.IncNumberOfRowsForType(typeof(T), this.VerifyData(headers));
			sw.Stop();
			toReturn.EnumerationTimeInMilliseconds = sw.Elapsed.TotalMilliseconds;
			if(!discardResults)
			{
				this._setBenchmarkResults.Add(toReturn);
			}
			return toReturn;
		}
		...
		{
			return this.PerformEagerLoadBenchmark(discardResults: false);
		}
		...
		public BenchResult PerformEagerLoadBenchmark(bool discardResults)
		{
			...
			var sw = new Stopwatch();
			sw.Start();
			var headers = this.FetchGraph();
			sw.Stop();
			toReturn.FetchTimeInMilliseconds = sw.Elapsed.TotalMilliseconds;
			sw.Reset();
			sw.Start();
			this.VerifyGraph(headers, toReturn);
			sw.Stop();
			if(!discardResults)
			{
				this._eagerLoadBenchmakResults.Add(toReturn);
			}
			return toReturn;
		}
		...
		protected string CreateFrameworkName(string formatString, Type t)
		{
			...
			string fileVersion;
			this.GetVersionStrings(EfBenchmarks.GetAssembly(t), out fileVersion, out assemblyVersion);
		...
		private void VerifyGraph(IEnumerable<T> toEnumerate, BenchResult resultContainer)
		{
			...
			foreach(var v in toEnumerate)
			{
				if(!this.VerifyGraphElement(v, resultContainer))
				{
		...
		private int VerifyElement(T toVerify)
		{
			...
			return this._salesOrderIdRetriever(toVerify);
		}


		private bool VerifyGraphElement(T parent, BenchResult resultContainer)
		{
			var toReturn = this.VerifyElement(parent);
			if(toReturn <= 0)
			{
				return false;
			}
			this.VerifyGraphElementChildren(parent, resultContainer);
